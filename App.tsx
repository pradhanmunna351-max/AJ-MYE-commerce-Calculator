...
import { Level, Region, ArticleType, ReverseLogisticsMode, BusinessBuffers, Marketplace, Brand, ManualRateCard } from './types';
...
const App: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'single' | 'batch' | 'ratecard'>('single');
          ...
          <div className="flex items-center gap-4">
            <div className="bg-forest-accent/30 p-1 flex rounded-xl border border-forest-accent/50 dark:bg-forest-leaf/10 dark:border-forest-leaf/20 shadow-inner">
              <button 
                ...
                className={`px-6 py-2 text-[10px] font-black uppercase rounded-lg transition-all ${activeTab === 'single' ? 'bg-forest-pine text-white shadow-md ring-1 ring-white/20 dark:bg-forest-leaf' : 'text-forest-pine/40 hover:text-forest-pine dark:text-forest-sage/60 dark:hover:text-forest-mint'}`}
              >
                Simulator
              </button>
              <button 
                onClick={() => setActiveTab('batch')} 
                className={`px-6 py-2 text-[10px] font-black uppercase rounded-lg transition-all ${activeTab === 'batch' ? 'bg-forest-pine text-white shadow-md ring-1 ring-white/20 dark:bg-forest-leaf' : 'text-forest-pine/40 hover:text-forest-pine dark:text-forest-sage/60 dark:hover:text-forest-mint'}`}
              >
                Bulk
              </button>
            </div>
